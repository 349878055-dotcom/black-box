...
## 6. 登录禁微信授权交付；哪条路通走哪条（用户铁令）

> 用户铁令：**禁止把「微信授权」当客户交付**。App 内置浏览器代替不了微信授权，这条路不走。
> 登录方式**没有写死必须浏览器**：按契约 `login.method`（`sms_verify` / `browser`），哪条路好走走哪条。

- **禁止** 建议 / 讨论 / 反复提及用「微信授权」（微信小程序 openid / wx.login code / 微信扫码授权）作为客户登录方案
- **禁止** 把「无法微信授权」当成登录障碍反复解释
- **必须** 看该 skill 的 `login.json`：短信 API 能通就 `sms_verify`；要真人网页就 `browser` 打开登录页 → 导出登录态存手机凭据库
- **自检**：提到某 skill 登录时，若脑子里冒出「微信授权」→ 停，改看契约走哪条路

## 7. 禁降级铁律（用户铁令 2026-08-16）

> 用户铁令：**所有降级全部除掉，禁止降级，不能掩盖问题。**（通用，适用于所有代码/链路）

- **禁止** 一切「降级 / 兜底 / fallback / 静默 `except: pass` / 失败返回 None 顶替」的处理
- **必须** 任何一步失败 → 直接报错 / 明确记日志，让问题暴露
- **自检**：写 try/except、兜底、降级、轮询兜底前：能不能去掉？能就去掉，让问题暴露

## 8. 回答从简铁律（用户铁令）

> 用户铁令：**能短说不要长说；简单回答；不要说其它话题。**

- **禁止** 长篇大论、绕来绕去、堆背景/堆术语把简单事讲复杂
