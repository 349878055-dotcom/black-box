...
# 找页面所有含"工程师/稍后/繁忙/失败/异常/太忙"的文本 + toast/弹层
js = ("(function(){var r=[];"
      "var kw=['工程师','稍后','繁忙','失败','异常','太忙','不可','暂','稍等','频繁','风控','验证'];" 
      "var a=document.querySelectorAll('*');"
      "for(var i=0;i<a.length&&i<50000;i++){var e=a[i];var t=(e.innerText||'').trim();"
      "if(t&&t.length<60){for(var j=0;j<kw.length;j++){if(t.indexOf(kw[j])>=0){r.push(t);break;}}}}"
      "return JSON.stringify([...new Set(r)].slice(0,20));})()")
...
def main():
    ...
    cmd = f"echo {b64} | base64 -d > /tmp/mt_toast.py && cd /home/ubuntu/xiami && python3 /tmp/mt_toast.py"
