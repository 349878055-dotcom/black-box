...
js = ("(function(){var a=document.querySelectorAll('*');"
      ...
      "if(t.indexOf('去结算')>=0||t.indexOf('提交订单')>=0||t.indexOf('结算')>=0){"
      "if(e.tagName==='BUTTON'||e.onclick||e.getAttribute('onclick')){e.click();return 'clicked:'+t.slice(0,20);}"
      "var c=e;for(var k=0;k<5;k++){if(c.onclick||c.tagName==='BUTTON'||c.getAttribute('onclick')){c.click();return 'clicked ancestor:'+t.slice(0,20);}c=c.parentElement;if(!c)break;}}}"
      "return 'notfound';})()")

print("CHECKOUT:", eval_js(js)[:200])
time.sleep(4)
...
def main():
    ...
    cmd = f"echo {b64} | base64 -d > /tmp/mt_checkout.py && cd /home/ubuntu/xiami && python3 /tmp/mt_checkout.py"
