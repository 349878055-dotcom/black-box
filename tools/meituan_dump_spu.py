#!/usr/bin/env python3
"""dump 美团 H5 菜单页一个菜品卡片的 HTML（找加购按钮）。"""
...
# dump 第一个 spu 卡片 outerHTML
js = ("(function(){var a=document.querySelectorAll('dd[data-tag=spu]');"
      "if(a.length>0)return a[0].outerHTML.slice(0,1800);return 'none';})()")
print(eval_js(js)[:2000])
'''

def main():
    b64 = base64.b64encode(REMOTE_PY.encode()).decode()
    cmd = f"echo {b64} | base64 -d > /tmp/mt_dump.py && cd /home/ubuntu/xiami && python3 /tmp/mt_dump.py"
