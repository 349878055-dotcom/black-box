"""
会话（Conversation）存储 — 单用户多会话管理（豆包式）。

会话归属 user_id（UUID），按用户隔离；消息直接存在会话记录里（消息即该会话的上下文）。
字段：conversation_id / user_id / title / type(chat|skill) / persona / messages / pinned / deleted
