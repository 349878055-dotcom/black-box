...
# ── ask_user 回答喂入注册表 ──
# email -> {ask_id: asyncio.Future}（同一设备可同时挂多个 ask，按 ask_id 精确路由；
...
def feed_answer(email: str, ask_id: str, value: str) -> bool:
    """喂回答给等待中的 ask_user 任务。匹配成功返回 True（调用方不要再当新消息处理）。"""
    if not email:
        return False
    waiters = _answer_waiter.get(email)
    if not waiters:
        return False
...
class MasterAgent:
    ...
    async def _run(self, email: str, user_id: str, conversation_id: str, message: str) -> None:
        ts = self._tasks.get(email)
        try:
            ask_fn = self._make_ask(email, user_id, conversation_id)
            steps_fn = self._make_steps(email, conversation_id)
            form_fn = self._make_forms(conversation_id)
            agent = Agent(ask_user_fn=ask_fn, email=email,
            ...
            if ts:
                ...
                ts.reply = reply
                ts.summary = "已完成"
                self._push_task(email)   # WS 推 task_update（done）
        except asyncio.CancelledError:
            if ts:
                ts.status = "cancelled"
                ts.summary = "已取消"
                self._push_task(email)   # WS 推 task_update（cancelled）
            ...
            if ts:
                ...
                ts.error = str(e)
                ts.summary = "处理失败"
                self._push_task(email)   # WS 推 task_update（failed）
        finally:
            self._busy.discard(email)

    def _make_ask(self, email: str, user_id: str, conversation_id: str):
        async def ask(question: str, image: str | None = None,
            ...
            _answer_waiter.setdefault(email, {})[ask_id] = fut
            if ts:
                ts.status = "waiting_user"
                ts.ask = {"question": str(question), "ask_id": ask_id, "options": opts}
                ts.summary = "等待你回复…"
                self._push_task(email)   # WS 推 task_update（waiting_user）
            # ask 提问记录到会话（bot 视角）
            self._append_conv(user_id, conversation_id,
            ...
            try:
                ...
                logger.exception("ask_user 推送失败 device=%s ask_id=%s: %s", email, ask_id, e)
                raise
            ...
            try:
                await bridge.send_push(email, "ask_done", {"ask_id": ask_id})
            except Exception:
                pass
            if ans:
                # 用户回答记录到会话（user 视角）
            ...
            if ts:
                ts.status = "running"
                ts.ask = None
                self._push_task(email)   # WS 推 task_update（回到 running）
            return (ans or "").strip()

        return ask

    def _make_steps(self, email: str, conversation_id: str = ""):
        ...
        def set_steps(steps: list[dict]) -> bool:
            ...
            try:
                ...
                logger.exception("会话进度落库失败 conv=%s device=%s", conversation_id, email)
                saved = False
            if ts:
                self._push_task(email)   # WS 推 task_update（带 steps 给 App 展示进度）
            return saved
    ...
    def _push_task(self, email: str) -> None:
        ...
        async def _push_safe():
            ...
            try:
                await _bridge.send_push(email, "task_update", ts.to_dict())
