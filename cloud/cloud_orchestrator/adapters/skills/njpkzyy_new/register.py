...
from .api import NjpkzyyNewAPI

_here = os.path.dirname(os.path.abspath(__file__))
_contract = json.load(open(os.path.join(_here, "contract.json"), encoding="utf-8"))

REGISTER: dict = {
    "id": _contract.get("id", "njpkzyy_new"),
    "name": _contract.get("name", "南京市浦口区中医院挂号"),
    "class": NjpkzyyNewAPI,                 # 函数实现类（api.py）
    "capability": _contract.get("capability", ""),
    "capability_note": _contract.get("capability_note", ""),
    "aliases": _contract.get("aliases", []),  # 别名（浦口中医院等客户说法命中）
