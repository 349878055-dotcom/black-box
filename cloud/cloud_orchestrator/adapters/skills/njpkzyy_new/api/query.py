"""njpkzyy_new · 查号源模块（公开接口，不用登录）。

list_depts / list_noon_codes / list_dept_doctors / get_available_dates / get_schedule /
judge_online / get_schedule_detail / get_pay_channels / list_notices / get_notice_detail。
"""
...
class QueryMixin:
    ...
    async def list_depts(self, branch_code: str = "") -> list[dict]:
        ...
        抓包实测：/api/public/basic/v3/depts 按 pid 组织父子科室，与小程序页面一致。
        """
        j = await self._get("/api/public/basic/v3/depts", {"branch_code": branch_code}, bearer=False)
        return self._out(j, [])
    ...
    async def list_noon_codes(self, branch_code: str = "", branch_name: str = "") -> list[dict]:
        ...
        j = await self._get("/api/public/v3/noon_code",
                            {"branch_code": branch_code, "branch_name": branch_name}, bearer=False)
        return self._out(j, [])
    ...
    async def list_dept_doctors(self, dept_code: str = "", branch_code: str = "",
        ...
        注意：本 skill 目录「科室」是两级结构（父科室→子科室），查医生/排班必须先
        list_depts 拿到目标科室的真实 dept_code 再传，禁止凭猜或套用示例数字。
        """
        path = f"/api/public/v3/dept/doctor/{dept_code}" if dept_code else "/api/public/v3/dept/doctor"
        j = await self._get(path, {"branch_code": branch_code, "branch_name": branch_name}, bearer=False)
        return self._out(j, [])
    ...
    async def get_available_dates(self, dept_code: str = "", begin_date: str = "",
        ...
        j = await self._get("/api/public/v3/schedule/check", {
    ...
    async def get_schedule(self, dept_code: str = "", begin_date: str = "",
        ...
        b = begin_date or t.isoformat()
        e = end_date or b
        j = await self._get("/api/public/v3/schedule", {
            "begin_date": b, "end_date": e, "dept_code": dept_code,
    ...
    async def judge_online(self, date: str, dept_code: str = "", doctor_code: str = "",
        ...
        """在线号判断（是否可挂在线号）。⚠️ 在线号/挂号类接口须用 ONLINE_AGENT_ID。"""
        j = await self._get("/api/public/v3/schedule/online/judge", {
            "date": date, "dept_code": dept_code, "doctor_code": doctor_code,
            "noon_code": noon_code, "time_type": time_type, "type": type,
        }, bearer=False, agent_id=ONLINE_AGENT_ID)
        return self._out(j, {})
    ...
    async def get_schedule_detail(self, schedule_id: str = "", schedule_day: str = "",
        ...
        """
        j = await self._get("/api/public/v3/schedule/online/detail", {
    ...
    async def get_pay_channels(self) -> list[dict]:
        ...
        j = await self._get("/api/public/v3/pay_channel", bearer=False)
        return self._out(j, [])

    async def list_notices(self, page: int = 0, size: int = 10) -> list[dict]:
        """公告/协议列表（/api/cms/news/category/3912）→ [{id, title, desc, createTime}]。"""
        j = await self._get("/api/cms/news/category/3912", {"page": page, "size": size}, bearer=False)
        return self._out(j, [])

    async def get_notice_detail(self, news_id: str = "") -> dict:
        """公告/协议详情（/api/cms/news/{news_id}）→ {id, title, text(html)}。"""
        j = await self._get(f"/api/cms/news/{news_id}", bearer=False)
        return self._out(j, {})
